...
(function($){
	...
	--------------------*/
	$(".element").typed({
		strings: ["Dorian Haupas", "Développeur Web"],
		typeSpeed: 75,
		loop:true,
		backDelay: 2000
	});
		...
		$.ajax({
			...
			success : function(text){
				if (text == "success"){
					...
					send_btn.text('Message envoyé !');
					/*setTimeout(function() {
						reset_form();
						send_btn.removeClass('done');
						send_btn.text('Envoyer le message');
					}, 10500);*/
				}
				else {
					/*alert(text);*/
					...
					send_btn.text('Une erreur s\'est produite.');
					setTimeout(function() {
